...
export function drawOrthographicProjections(
  ...
  if (!Array.isArray(landmarksArray[0])) {
    landmarksArray = [landmarksArray as Landmark[]];
  }

  (landmarksArray as Landmark[][]).forEach((landmarks: Landmark[]) => {
    ...
    if (connections) {
      ...
      ctx.lineWidth = 2;
      connections.forEach((connection: { start: number; end: number }) => {
